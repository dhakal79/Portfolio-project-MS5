    ...
    if weight:
        if item_id in list(bag.keys()):
            if weight in bag[item_id]['items_by_weight'].keys():
                ...
                messages.success(request, f'Updated weight {weight.upper()} \
                    {product.name} to \
                        {bag[item_id]["items_by_weight"][weight]}')
            else:
                bag[item_id]['items_by_weight'][weight] = quantity
                messages.success(request, f'Added weight {weight.upper()} \
        ...
        if item_id in list(bag.keys()):
            ...
            messages.success(request, f'Update {product.name} \
    ...
    if weight:
        if quantity > 0:
            ...
            if not bag[item_id]['items_by_weight']:
                bag.pop(item_id)
            messages.success(request, f'Removed weight {weight.upper()} \
